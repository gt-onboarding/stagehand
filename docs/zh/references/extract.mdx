...
<Tabs>
  <Tab title="With Schema">
    **返回：** `Promise<ExtractResult<T>>`，其中 T 与您的 schema 匹配

    返回对象将严格按照您的 schema 定义进行类型约束。
  </Tab>

  <Tab title="String Only">
    **返回：** `Promise<{ extraction: string }>`

    不进行 schema 验证的简单字符串提取。
  </Tab>
  ...
  <Tab title="Advanced">
    <CodeGroup>
      ...
      const ProductSchema = z.object({
        ...
        available: z.boolean().describe("是否有现货"),
      ...
      class Product(BaseModel):
          ...
          available: bool = Field(description="是否有现货")
