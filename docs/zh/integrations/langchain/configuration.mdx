...
sidebarTitle: 配置
description: "将 Stagehand 与 LangChain JS 配置结合，创建智能网页自动化代理"
---

本指南将带你将 Stagehand 与 LangChain JS 集成起来，并使用自然语言指令构建强大的网页自动化流程。
...
用于远程浏览器自动化，请设置你的 Browserbase 凭据：
...
- **stagehand_observe**：分析页面元素与可执行动作
...
try {
    const result = await agent.invoke({
        messages: [{ role: "user", content: "访问 invalid-url.com" }]
    });
} catch (error) {
...
<Tabs>
  <Tab title="数据抽取" value="data-extraction" label="TypeScript">
    ```typescript
  ...
  <Tab title="多站点调研" value="multi-site-research" label="TypeScript">
    ...
    const result = await researchAgent.invoke({
        messages: [{
            ...
            content: `
                进行产品定价调研，步骤：
                1. 访问 competitor1.com 并抽取定价信息
                2. 访问 competitor2.com 并抽取定价信息
                3. 比较功能与价格
                4. 提供总结性分析
            `
        }]
