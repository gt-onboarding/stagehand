...
sidebarTitle: 配置
description: Next.js 是一款在生产环境中广泛用于构建 Web 应用的流行框架。它为诸如 [Director](https://director.ai)、[Brainrot](https://brainrot.run) 和 [Open Operator](https://operator.browserbase.com) 等 Stagehand 应用提供支持。
---

<Card
  title="查看 Stagehand 的 Next.js 快速入门"
  icon="github"
  href="https://github.com/browserbase/stagehand-nextjs-quickstart"
>
  克隆我们的 [GitHub 仓库](https://github.com/browserbase/stagehand-nextjs-quickstart) 以在 Next.js 中开始使用 Stagehand。
</Card>

## 将 Stagehand 添加到现有的 Next.js 项目
...
### 编写一个服务器端 action
接下来，在 `app/stagehand/main.ts` 中将 `main` 函数定义为服务器端 action。该文件将包含以下三个函数：
...
async function main(stagehand: Stagehand) {
  // 你可以使用 `page` 实例编写任意 Playwright 代码
  // 更多信息：https://playwright.dev/docs/pom
  const page = stagehand.page;

  // 在这个示例中，我们将获取 Stagehand 快速上手页面的标题
...
### 创建一个客户端组件
接下来，创建一个客户端组件，用于启动 Browserbase 会话，并使用刚定义的服务器端 actions 来运行 `main` 函数。我们会先创建一个 Browserbase 会话，将其通过 iframe 嵌入页面，然后再运行 `main` 函数。
...
现在，我们就可以在应用中使用 `StagehandEmbed` 组件了。
