...
sidebarTitle: 配置
description: Next.js 是一个在生产环境中广泛用于构建 Web 应用的流行框架。它为 [Director](https://director.ai)、[Brainrot](https://brainrot.run) 和 [Open Operator](https://operator.browserbase.com) 等 Stagehand 应用提供支持。
---
...
如果你想把 Stagehand（AI 增强浏览器自动化框架）集成到现有的 Next.js 项目中，可以先安装以下依赖：
<Tabs>
	<Tab title="npm">
	```bash
...
### 编写服务器 action
接下来，在 `app/stagehand/main.ts` 中将 `main` 函数定义为服务器 action。该文件将包含以下三个函数：

1. **`main`：运行 Stagehand 的主脚本**
...
### 创建客户端组件
接下来，创建一个客户端组件，用于启动 Browserbase 会话，并使用刚定义的服务器 action 运行 `main` 函数。我们会先创建 Browserbase 会话，将其通过 iframe 嵌入页面，然后再运行 `main` 函数。
...
现在，你可以在应用中使用 `StagehandEmbed` 组件。
...
<CardGroup cols={2}>
  <Card title="Deploy Template" icon="rocket" href="https://vercel.com/templates/ai/stagehand-next-js-quickstart">
    一键在 Vercel 上部署 Stagehand Next.js 模板
  </Card>
