...
许多现代 Web 应用在用户点击某些按钮或链接时会打开新的标签页。若缺乏完善的多标签页支持，当预期内容出现在新标签页而非当前标签页时，自动化脚本就可能失败。Stagehand 的多标签页能力可确保你的自动化在多标签页流程中顺畅运行。

## Stagehand 的 Page

Stagehand 会自动适配多标签页工作流。`stagehand.page` 对象始终指向最近打开或当前活动的标签页，确保即使创建了新标签页，你的自动化也能继续运行。
...
<Warning>
  **重要**：[Stagehand Agent](/zh/basics/agent) 始终作用于 `stagehand.page`。如果你需要让代理跨特定标签页工作，你需要手动管理页面切换。
</Warning>

## 手动管理页面

为了获得更精细的控制或实现多标签页工作流，您可以显式管理多个标签页：
