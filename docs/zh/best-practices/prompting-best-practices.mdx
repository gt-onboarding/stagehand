...
### 使用元素类型而非颜色 {#use-element-types-not-colors}
...
### 使用更具描述性的语言 {#use-descriptive-language}
...
### 选择正确的动作动词 {#choose-the-right-action-verbs}
...
### 正确处理数组 {#handle-arrays-correctly}
...
### 使用合适的 URL 类型 {#use-proper-url-types}
...
## observe 方法 {#observe-method}

在执行操作之前，使用 `observe()` 先发现可操作的元素。

### 先检查元素 {#check-elements-first}
...
### 明确指定元素类型 {#be-specific-about-element-types}
...
## Agent 方法 {#agent-method}

对复杂的多步工作流使用 `agent()`。提供详尽的指令并设置恰当的限制。

### 先进行导航 {#navigate-first}
...
### 设置合适的步数上限 {#set-appropriate-step-limits}
...
### 包含成功判定标准 {#include-success-criteria}
...
## 常见错误及规避方法 {#common-mistakes-to-avoid}
