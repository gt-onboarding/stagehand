...
title: 成本优化  
sidebarTitle: 成本优化
description: 在保持自动化性能的同时将成本最小化
---

在 Stagehand 中进行成本优化需要在 LLM 推理成本与浏览器基础设施成本之间取得平衡。本文提供实用策略，帮助降低自动化成本。
...
我们不建议在简单任务上使用更大、更高价的模型。查看我们的[评估结果](https://stagehand.dev/evals)，了解不同任务类型下的模型性能与成本对比。
...
缓存成功的操作以避免重复调用 LLM。可在我们的[缓存指南](/zh/best-practices/caching)中了解基础用法：
...
尽可能复用会话并设置合理的超时。详见[浏览器配置](/zh/configuration/browser)：
...
<CodeGroup>
  ```typescript TypeScript
  // 根据任务复杂度从低到高选择模型
  // 参见 stagehand.dev/evals 获取性能/成本对比
