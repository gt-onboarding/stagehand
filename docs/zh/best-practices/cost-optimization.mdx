...
在 Stagehand 中的成本优化是指在 LLM 推理成本与浏览器基础设施成本之间取得平衡。本指南提供切实可行的策略，帮助你降低自动化成本。
...
我们不建议为简单任务使用更大、更昂贵的模型。查看我们的[评估结果](https://stagehand.dev/evals)，比较不同任务类型下的模型表现与成本。
...
缓存成功的动作以避免重复的 LLM 调用。请在我们的[缓存指南](/zh/best-practices/caching)中了解基础知识：
...
尽可能复用会话并设置合适的超时时间。详见[浏览器配置](/zh/configuration/browser)：
...
跟踪开销以发现优化空间。有关详细指标，请参阅我们的[可观测性指南](/zh/configuration/observability)：
