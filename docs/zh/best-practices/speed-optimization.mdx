...
title: 速度优化
sidebarTitle: 速度优化
description: 优化 Stagehand 性能，加速自动化并降低延迟
---
...
<CodeGroup>
  ...
  for (const field of formFields) {
    await page.act(field); // 无需 LLM 调用！
  }
...
<Note>
  **性能提示**：基于 `observe` 结果执行操作可完全避免 LLM 推理。该方式比直接调用 `act()` 快 2–3 倍，是多步工作流的推荐模式。
</Note>
...
<CodeGroup>
  ...
  await page.goto("https://heavy-spa.com", {
    waitUntil: "domcontentloaded", // 不必等待所有资源
...
<CodeGroup>
  ...
  // 使用更快的导航策略
  await page.goto(url, { 
    waitUntil: 'domcontentloaded',  // 不等待图片/字体加载
    timeout: 10000 
  });
  ```
  ...
  await page.goto(
      url, 
      wait_until='domcontentloaded',  # 不等待图片/字体加载
      timeout=10000
  )
  ```
</CodeGroup>
