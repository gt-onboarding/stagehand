...
Iframes 会在当前页面中嵌入其他页面。网站常将其用于同意/隐私横幅、支付组件、聊天气泡以及第三方内容。
iframe 内的元素与主页面处于不同的上下文。
...
- 当你不确定某个元素是否位于 iframe 中时，可在 Stagehand 日志中检查是否存在 iframes。
- 如果某个元素偶尔无法被找到，可能位于延迟加载的 iframe 中。可在步骤之间加入短暂等待或重试该操作。

<Note>
  你可以通过 Stagehand 配置启用实验性功能（例如 Shadow DOM 支持）。参见[配置指南](/zh/configuration/browser)。
</Note>
...
<CardGroup cols={2}>
  <Card title="使用 observe() 分析页面" icon="magnifying-glass" iconType="sharp-solid" href="/zh/basics/observe">
    使用 `observe()` 在执行前规划精确的单步操作。
  </Card>

  <Card title="使用 extract() 提取数据" icon="table" iconType="sharp-solid" href="/zh/basics/extract">
    将 `extract()` 与数据模式配合使用，从任意页面提取干净、具类型的数据。
  </Card>

  <Card title="操作缓存" icon="bolt" iconType="sharp-solid" href="/zh/best-practices/caching">
    通过缓存操作，加速重复的自动化流程。
  </Card>

  <Card title="act() 基础" icon="arrow-pointer" iconType="sharp-solid" href="/zh/basics/act">
    了解如何使用 `act()` 可靠地执行单步操作。
  </Card>
</CardGroup>