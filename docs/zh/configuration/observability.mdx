...
Stagehand 提供强大的可观测性功能，帮助你监控、跟踪性能并分析浏览器自动化流程。聚焦会话监控、资源使用，以及在 Browserbase 与本地环境中的运营洞察。

<div id="browserbase-session-monitoring">
  ## Browserbase 会话监控
</div>

在 Browserbase 上运行时，你可以通过 Browserbase API 和仪表板获得完善的云端监控与会话管理能力。
...
<CardGroup>
  ...
  <Card title="Session Recordings" icon="video">
    查看完整会话录制并逐帧回放。以可视化方式分析网络请求并调试浏览器交互。
  </Card>
  ...
  <Card title="Usage Monitoring" icon="chart-bar">
    跟踪资源消耗、会话时长与 API 使用情况。获取关于自动化流程的成本与利用率的详细拆解。
  </Card>
</CardGroup>
...
在本地开发中，Stagehand 可在你的机器上直接提供性能监控与资源跟踪能力。
...
<Card title="LLM Usage" icon="chart-line" href="/zh/configuration/evals">
  监控 Token 使用量、成本与速度；为关键故障设置自动告警；在不同环境中进行成本跟踪；利用会话分析优化自动化工作流。
</Card>
...
metrics 对象会按 Stagehand 的各项操作提供详细拆分：
...
你也可以将 `logInferenceToFile` 设置为 `true`，把推理过程记录到文件。这将在项目根目录创建名为 `inference_summary` 的目录。
...
有关详细的日志与调试功能，请参阅 [Logging](/zh/configuration/logging)。