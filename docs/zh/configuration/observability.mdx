...
Stagehand 提供强大的可观测性功能，帮助你监控、评估性能，并分析浏览器自动化工作流。重点覆盖会话监控、资源使用，以及在 Browserbase 与本地环境中的运营洞察。

## Browserbase 会话监控

在 Browserbase 上运行时，您可以通过 Browserbase API 和仪表盘获得全面的云端监控与会话管理能力。
...
<CardGroup>
  <Card title="Real-Time Monitoring" icon="chart-line">
    监控实时会话状态、资源使用与地理分布。基于实时洞察扩缩与管理并发会话。
  </Card>

  <Card title="Session Recordings" icon="video">
    回看完整会话录制并逐帧播放。可视化分析网络请求并调试浏览器交互。
  </Card>

  <Card title="API Management" icon="code">
    通过我们的 API 以编程方式访问会话数据、自动化生命周期管理，并与监控系统集成。
  </Card>

  <Card title="Usage Monitoring" icon="chart-bar">
    跟踪资源消耗、会话时长与 API 使用情况。获取自动化全局的成本与利用率明细。
  </Card>
</CardGroup>
...
用于本地开发时，Stagehand 可在你的机器上直接提供性能监控与资源跟踪能力。
...
<Card title="LLM Usage" icon="chart-line" href="/zh/configuration/evals">
  监控 token 用量、成本与速度。为关键故障设置自动告警。在不同环境中实现成本跟踪。使用会话分析优化自动化工作流。
</Card>
...
使用 `stagehand.metrics` 实时监控自动化流程的资源使用情况：
...
metrics 对象按 Stagehand 各项操作提供详细拆分：
...
也可以将 `logInferenceToFile` 设置为 `true`，把推理过程记录到文件中。此操作会在项目根目录创建名为 `inference_summary` 的目录。
...
有关详细的日志与调试功能，请参见 [Logging](/zh/configuration/logging)。