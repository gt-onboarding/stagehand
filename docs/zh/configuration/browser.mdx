...
Stagehand 支持两种主要运行环境：

- **Browserbase** - 面向大规模生产环境的云托管浏览器基础设施，专为网页自动化优化
- **本地** - 在本机直接运行浏览器，用于开发与调试

## Browserbase 环境

Browserbase 提供面向大规模 Web 自动化优化的托管云浏览器基础设施，具备隐身模式、代理支持和持久化上下文等高级功能。

<Card icon="cloud" title="Browserbase" href="https://docs.browserbase.com" description="了解在可扩展 Web 自动化中使用 Browserbase 的功能与优势。">
  使用 Browserbase，体验云托管浏览器基础设施的强大能力。
</Card>
...
<Accordion title="高级 Browserbase 配置示例">
    ...
    const stagehand = new Stagehand({
      ...
      browserbaseSessionCreateParams: {
        ...
        browserSettings: {
          advancedStealth: false, // 这是 Scale 方案功能——请联系 support@browserbase.com 启用
          blockAds: true,
          solveCaptchas: true,
          recordSession: false,
          viewport: {
    ...
    stagehand = Stagehand(
        ...
        browserbase_session_create_params={
            ...
            "browser_settings": {
                "advanced_stealth": False,  # 这是 Scale 方案功能——请联系 support@browserbase.com 启用
                "block_ads": True,
                "solve_captchas": True,
...
**打开 Browserbase 的[会话实时视图](https://docs.browserbase.com/features/session-live-view)**，以引入人工在环。
</Accordion>
...
[Browserbase 会话](https://docs.browserbase.com/introduction/what-is-browserbase)的**唯一标识符**。用于在 Browserbase 控制台中标识会话并连接到该会话。
</Accordion>
</AccordionGroup>

### 备选方案：Browserbase SDK

如果你更希望直接管理会话，可以使用 Browserbase SDK：
...
本地环境直接在你的机器上运行浏览器，能够完全控制浏览器实例与配置。适合用于开发、调试，以及需要自定义浏览器设置的场景。
...
连接到现有的本地 Chrome/Chromium 浏览器，而不是启动一个新的实例。这样你可以在日常使用的浏览器上进行自动化，保留所有现有标签页、扩展和设置。
...
- 检查你的 API Key 是否具备必要权限
- 确保你的 Browserbase 账户有足够额度
</Accordion>
