...
title: 日志与调试
sidebarTitle: 日志
description: 为 Stagehand 工作流配置日志、调试与错误追踪
---
...
- **级别 0**：仅错误——适用于生产环境的最小输出
- **级别 1**：信息——包含成功操作与重要事件
- **级别 2**：调试——包含内部操作在内的全面日志
...
- 将 `verbose` 设为 `2` 并启用可视化调试
- 启用浏览器 DevTools 检查元素
- 使用 `logInferenceToFile: true` 以捕获 LLM 决策
- 尽早实施结构化日志
</Accordion>

<Accordion title="生产环境">
- 将 `verbose` 设为 `1`，在可见性与性能之间取得平衡
- 实施错误跟踪与告警
- 使用结构化 JSON 日志
- 监控会话成功率与执行时长
</Accordion>
