...
使用 TypeScript 或 Python SDK 将 Stagehand 安装到你现有的应用中。

<Tip>
  对于 TypeScript/Node.js：我们强烈建议在 Node.js 运行时中运行 Stagehand 脚本，而非使用 Deno 或 Bun 等较新的替代方案。
...
<Tabs>
  <Tab title="TypeScript">
    ...
    <Tip>
      如果计划本地运行，请先执行一次浏览器安装：`npx playwright install`。
      云端浏览器会话可跳过此步骤。
    </Tip>
...
<CardGroup cols={2}>
  <Card title="Configuration" icon="gear" href="/zh/configuration/browser">
    环境、Browserbase 与本地、日志、超时设置、LLM 定制
  </Card>
